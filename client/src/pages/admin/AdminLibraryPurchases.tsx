...
export default function AdminLibraryPurchases() {
  ...
  const reviewMutation = useMutation({
    mutationFn: async ({ id, action }: { id: number; action: "approve" | "reject" }) => {
      ...
      if (!res.ok) throw new Error(t("library.actionFailed"));
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/library/purchases/all"] });
      toast({ title: t("library.done") });
    },
  ...
  const viewReceipt = async (id: number) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t("library.receiptOpenFailed"));
  ...
  const statusBadge = (s: string) => {
    if (s === "PENDING") return <Badge className="bg-amber-500">{t("library.underReview")}</Badge>;
    if (s === "APPROVED") return <Badge className="bg-green-600">{t("library.approved")}</Badge>;
    return <Badge variant="destructive">{t("library.rejected")}</Badge>;
  };

  return (
    <DashboardLayout title={t("library.purchasesTitle")}>
                ...
                <CardContent className="py-4 space-y-3">
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    <div className="min-w-0">
                      <p className="font-medium">{r.fileTitle || t("library.deletedFile")}</p>
                      <p className="text-sm text-muted-foreground">
                        {[r.studentFirst, r.studentLast].filter(Boolean).join(" ")} • {r.studentEmail}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-primary">{r.filePrice ? `${r.filePrice} JOD` : t("library.free")}</span>
                    ...
                    {r.status === "PENDING" && (
                      <>
                        <Button size="sm" className="bg-green-600 hover:bg-green-700"
                          ...
                          <Check className="w-4 h-4 ml-1" /> {t("library.accept")}
                        </Button>
                        <Button size="sm" variant="destructive"
                          onClick={() => reviewMutation.mutate({ id: r.id, action: "reject" })}
                          disabled={reviewMutation.isPending}>
                          <X className="w-4 h-4 ml-1" /> {t("library.reject")}
                        </Button>
                      </>
                    )}
                  </div>
